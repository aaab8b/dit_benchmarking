...
def main(args):
    ...
    train_steps = 0
    for train_steps, (x, y) in enumerate(loader):
        x = x.to(device)
        y = y.to(device)
        with torch.no_grad():
        ...
        # np.save(f'{args.features_path}/imagenet256_labels/{train_steps}_rank{rank}.npy', y)
        np.save(f'{args.features_path}/imagenet256_features/{train_steps}_rank{rank}_label{y[0]}.npy', x)
